---
orphan: true
html_meta:
  description: 1.1.1 (February 2019). Anbox was taking an incorrect display size from
    platform plugins and failed to initialize EGL rendering context. The Anbox container
    now.
---
# 1.1.1 (February 2019)

## Bug fixes

* Anbox was taking an incorrect display size from platform plugins and failed to initialize EGL rendering context.
* The Anbox container now always dumps system log files when an error occurred.