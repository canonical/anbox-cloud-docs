...
* [LP 2114950](https://bugs.launchpad.net/anbox-cloud/+bug/2114950) Even when the application is set to use landscape mode at startup, the orientation used remains in portrait mode. This issue occurs when the screen rotation event is not triggered. To work around this issue until we release a fix, you can use a pre-start [hook](https://documentation.ubuntu.com/anbox-cloud/reference/hooks/) that clears the configuration. Here's an example of how the pre-start hook could look like:
