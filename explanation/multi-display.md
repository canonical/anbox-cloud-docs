...
The SDK retains its existing single-display target-element option. Multi-display consumers use per-track attachment instead. These compatibility paths preserve single-display behaviour. They do not give older runtimes or clients multi-display support.
