...
```{note}
In Anbox Cloud's 1.25.0 release, the lxd charm went through a rework which led to a number of changes in the upgrade instructions for the charm.
```
...
The first step to upgrade LXD is to update the charm itself to the latest revision using:

    juju refresh --channel=1.25/stable lxd

Updating the charm does not include updating the LXD snap or any of the internal packages on the LXD node. After updating the charm, check which nodes have package updates available using:
...
Before proceeding make sure that all the traffic to nodes that have updates available have been drained. Run:

    juju run ams/leader start-maintenance node=”<node_name>”

where `<node_name>` refers to the LXD node name stored within AMS. This action marks the LXD node as unschedulable within AMS which would make sure that no new instances are spawned on the LXD node.

After the node is successfully marked, run:
...
After the updates to all the nodes finish, update the LXD snap on the node using:

    juju run lxd/<unit_number> upgrade-cluster.

```{important}
For customers running LXD clusters with the LXD Snap tracking a channel which is different than 5.21/stable, please set the charm configuration item `channel` *explicitly* to the currently running channel before running the `upgrade-cluster` command. This can be done using:
...
Finally after all the upgrades finish and the nodes are healthy, run:
    ...
    juju run ams/leader end-maintenance node=”<node_name>”
