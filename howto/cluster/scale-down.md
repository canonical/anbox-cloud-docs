...
The following are important requirements when scaling down:
 ...
 - Unless you are removing the entire cluster or are prepared for breaking changes, you should **not**
   ...
   - remove the leader LXD unit. You can identify the LXD leader unit by running `juju status lxd`. The leader is indicated by an asterisk (*) appended to the unit name in the output.
 - If the LXD leader is in the process of rebooting due to a kernel module or GPU driver update, do not scale down the LXD cluster until the leader has fully rebooted or Juju elects a new LXD unit as leader.
