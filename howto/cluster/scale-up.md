...
```{important}
Since Anbox Cloud 1.25.0, the default channel for LXD charm was changed to 5.21/stable.
For customers running LXD clusters with the LXD Snap tracking a channel which is different than 5.21/stable, please set the charm configuration item `channel` *explicitly* to the currently running channel before scaling up or down. This can be done using:
