...
Create the file `my-android-snap/meta/snap.yaml`:
    ...
    cat << 'EOF' > my-android-snap/meta/snap.yaml
...
Launch a virtualized Android instance in development mode so that you can push files to it:
    ...
    id=$(amc launch --devmode --raw resolute:android16-cf:amd64 --memory 5GB)

Wait for the instance to reach the `running` state:

    amc wait "${id}" --timeout 5m

See {ref}`sec-dev-mode` for more information about development mode.

Push the snap into the instance:

    lxc file push my-custom-android_1.0_amd64.snap "ams-${id}/tmp/"
...
Disconnect the existing Android snap from the runtime and connect your custom snap:

    # Disconnect the original Android snap
    amc exec "${id}" -- snap disconnect anbox-runtime:android-images
    amc exec "${id}" -- snap disconnect anbox-runtime:android-host-tools

    # Connect the custom Android snap
    ...
    # Restart the runtime to pick up the new Android images
